...
abstract contract Abs_FraudContributor is Lib_AddressResolver {
    ...
    modifier contributesToFraudProof(uint256 _chainId, bytes32 preStateRoot, bytes32 txHash) {
        uint256 startGas = gasleft();
        _;
        uint256 gasSpent = startGas - gasleft();
        iOVM_BondManager(resolve("OVM_BondManager"))
            .recordGasSpentByChainId(_chainId, preStateRoot, txHash, msg.sender, gasSpent);
    }
}
