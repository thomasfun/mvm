...
def start_watchdog(init_system):
    if init_system == 'entrypoint.sh' or init_system == 'sh' or init_system == 'systemd':
        init_system = 'service'
    ...
    elif init_system == 'service':
        rc = subprocess.check_output(f'ps aux|grep {WATCHDOG_SERVICE}|grep -v grep|wc -l', shell=True)
        if rc is not None and int(rc.decode('utf-8').replace('\n','')) < 1:
            subprocess.check_output(f'/sbin/start-stop-daemon -b -q --start --exec /usr/bin/{WATCHDOG_SERVICE}', shell=True)
        else:
            logging.debug('%s is already running', WATCHDOG_SERVICE)
